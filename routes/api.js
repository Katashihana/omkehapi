...
const token = ["utaaavii", "utaa", "avii"];

router.post("/addapikey", async (req, res, next) => {
  const key = req.query.key;
  const token = req.query.token;
  if(!token) return res.json(loghandler.nottoken)
...
router.get('/maker/tololserti', async(req, res, next) => {
  ...
  if(!apikey) return res.json(loghandler.notparam)
  if(listkey.includes(apikey)){
  let hasil = `https://lolhuman.herokuapp.com/api/toloserti?apikey=PinnBotWibu&name=${text}`
  data = await fetch(hasil).then(v => v.buffer())
